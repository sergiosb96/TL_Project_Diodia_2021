const PassesCost = require("../../cli/src/Scopes/PassesCost");
...
// Unit tests for PassesCost class
describe("PassesCost class unit testing", () => {
  const scope = new PassesCost();

  // getName method
  test("getName() getter method", () => {
    expect(scope.getName()).toBe("passescost");
  });
  ...
  test("run() method with valid parameters", async () => {
    ...
    // PassesCost class is expected to make a GET request
    axios.get.mockResolvedValueOnce(fakeResponse);

    // run PassesCost's main function
    const result = await scope.run(baseUrl, parameters);

    // check if axios was called with the expected URL
    expect(axios.get).toHaveBeenCalledWith(
      `${baseUrl}/passescost/aodos/kentriki_odos/20200101/20200301?format=json`,
      undefined
    );
