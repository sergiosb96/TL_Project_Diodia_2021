...
// Form : Select operator, station and dates from/to
const FormOperatorStationDate = (props) => {
  ...
  // get find selected operator's object by ID, from passed props
  const selectedOperator =
    operatorIsValid && props.operators.find((op) => op.id === operator);

  // selected station Id
  const [selectedStation, setSelectedStation] = useState(
    selectedOperator && selectedOperator.stations.length > 0
      ? selectedOperator.stations[0].id
      : "Δε βρέθηκαν σταθμοί"
  );
  const selectedStationIsValid =
    operatorIsValid &&
    selectedOperator &&
    selectedStation &&
    selectedOperator.stations.findIndex(
      (station) => station.id === selectedStation
    ) !== -1;

    // do not allow null (or invalid) selected station if there is an available valid station
    if(!selectedStationIsValid && operatorIsValid && selectedOperator.stations.length > 0) {
      setSelectedStation(selectedOperator.stations[0].id);
    }
  ...
  return (
    ...
    >
      <Row>
        ...
        <Col xs={12} md={6}>
          <Form.Label htmlFor="select-station" className="text-center">
            Επιλογή Σταθμού
          </Form.Label>
          <Form.Select
            aria-label="Σταθμός"
            name="select-station"
            isInvalid={!selectedStationIsValid}
            disabled={isLoading}
            value={selectedStation}
            onChange={(event) => setSelectedStation(event.target.value)}
          >
            {selectedOperator &&
              selectedOperator.stations.length > 0 &&
              selectedOperator.stations.map((station) => (
...
export default FormOperatorStationDate;
